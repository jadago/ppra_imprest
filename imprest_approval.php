        ...
        <div class="page-container container-fluid">
            ...
            <div class="page-content">
                ...
                <div class="page-title">
                    <h5><i class="fa fa-warning"></i> Imprest Management</h5>
                </div>
                <!-- /page title -->
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="panel panel-info">
                            <div class="panel-heading"><h6 class="panel-title">New Imprest Request - For Approval</h6></div>
                            <div class="datatable">
                                <table class="table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        {
                                        $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=1) ORDER BY imprest_id DESC";
                                        }
                                        elseif($arraylogUser['department']==2)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=2) ORDER BY imprest_id DESC";  
                                        }
                                        elseif($arraylogUser['department']==3)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=3) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==4)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=4) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==5)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=5) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==6)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE status > 2 AND addedby IN(SELECT user_id FROM users WHERE department=6) ORDER BY imprest_id DESC";  
                                        }
                                        ...
                                        while ($result = mysqli_fetch_array($query)) {
                                           ...
                                           // get total amount
                                            ...
                                            <tr>
                                                ...
                                                <td><?php if($result['status']==3){?><a href="imprest_preview_director.php?id=<?php echo $result['imprest_id'];?>"><span class="label label-primary">Approve this imprest</span></a><?php }elseif($result['status']==4) {?>
                                                <span class="label label-success">Approved</span><?php } elseif($result['status']==5){?><span class="label label-success">Paid</span><?php } else {?><span class="label label-warning">Rejected</span><?php }?></td>
