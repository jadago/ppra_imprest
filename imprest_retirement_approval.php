                            ...
                            <div class="datatable">
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        {
                                        $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=1) ORDER BY imprest_id DESC";
                                        }
                                        elseif($arraylogUser['department']==2)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=2) ORDER BY imprest_id DESC";  
                                        }
                                        elseif($arraylogUser['department']==3)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=3) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==4)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=4) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==5)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=5) ORDER BY imprest_id DESC";  
                                        }
                                            elseif($arraylogUser['department']==6)
                                        {
                                         $getitem = "SELECT * FROM imprest WHERE stage > 2 AND addedby IN(SELECT user_id FROM users WHERE department=6) ORDER BY imprest_id DESC";  
                                        }
                                        ...
                                        while ($result = mysqli_fetch_array($query)) {
                                           ...
                                           // get total amount
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                <?php
                                                if($result['stage']==3){?><a href="imprest_retirement_approval_preview.php?id=<?php echo $result['imprest_id'];?>"><span class="label label-success">Preview & Approve</span></a><?php }
                                                elseif($result['stage']>3){?><span class="label label-success">Approved</span><?php }?>
                                                </td>
